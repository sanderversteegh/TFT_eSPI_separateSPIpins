...
inline void TFT_eSPI::begin_touch_read_write(void){
  ...
  CS_H; // Just in case it has been left low
    ...
    #if defined (USE_SEPERATE_TOUCH_BUS)
      if (locked) {locked = false; Tspi.beginTransaction(SPISettings(SPI_TOUCH_FREQUENCY, MSBFIRST, SPI_MODE0));}
    #else
      if (locked) {locked = false; spi.beginTransaction(SPISettings(SPI_TOUCH_FREQUENCY, MSBFIRST, SPI_MODE0));}
    #endif
  #else
    #if defined (USE_SEPERATE_TOUCH_BUS)
      Tspi.setFrequency(SPI_TOUCH_FREQUENCY);
    #else
...
inline void TFT_eSPI::end_touch_read_write(void){
  ...
  #if defined (SPI_HAS_TRANSACTION) && defined (SUPPORT_TRANSACTIONS)
    #if defined (USE_SEPERATE_TOUCH_BUS)
      if(!inTransaction) {if (!locked) {locked = true; Tspi.endTransaction();}}
    #else
      if(!inTransaction) {if (!locked) {locked = true; spi.endTransaction();}}
    #endif
  #else
    #if defined (USE_SEPERATE_TOUCH_BUS)
      tSPI.setFrequency(SPI_FREQUENCY);
    #else
...
uint8_t TFT_eSPI::getTouchRaw(uint16_t *x, uint16_t *y){
  ...
  begin_touch_read_write();
    ...
    Tspi.transfer(0xd0);                    // Start new YP conversion
    Tspi.transfer(0);                       // Read first 8 bits
    Tspi.transfer(0xd0);                    // Read last 8 bits and start new YP conversion
    Tspi.transfer(0);                       // Read first 8 bits
    Tspi.transfer(0xd0);                    // Read last 8 bits and start new YP conversion
    Tspi.transfer(0);                       // Read first 8 bits
    Tspi.transfer(0xd0);                    // Read last 8 bits and start new YP conversion

    tmp = Tspi.transfer(0);                   // Read first 8 bits
    tmp = tmp <<5;
    tmp |= 0x1f & (Tspi.transfer(0x90)>>3);   // Read last 8 bits and start new XP conversion

    *x = tmp;

    // Start XP sample request for y position, read 4 times and keep last sample
    Tspi.transfer(0);                       // Read first 8 bits
    Tspi.transfer(0x90);                    // Read last 8 bits and start new XP conversion
    Tspi.transfer(0);                       // Read first 8 bits
    Tspi.transfer(0x90);                    // Read last 8 bits and start new XP conversion
    Tspi.transfer(0);                       // Read first 8 bits
    Tspi.transfer(0x90);                    // Read last 8 bits and start new XP conversion
    ...
    tmp = Tspi.transfer(0);                 // Read first 8 bits
    tmp = tmp <<5;
    tmp |= 0x1f & (Tspi.transfer(0)>>3);    // Read last 8 bits
...
uint16_t TFT_eSPI::getTouchRawZ(void){
  ...
  begin_touch_read_write();
    ...
    // Z sample request
    int16_t tz = 0xFFF;
    Tspi.transfer(0xb0);               // Start new Z1 conversion
    tz += Tspi.transfer16(0xc0) >> 3;  // Read Z1 and start Z2 conversion
    tz -= Tspi.transfer16(0x00) >> 3;  // Read Z2
  #else
    // Z sample request
    int16_t tz = 0xFFF;
